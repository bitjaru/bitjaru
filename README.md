...
`Unity` `WebGL` `C#` `Kotlin backend` `Telegram mini-app` `LINE mini-app` `on-chain rewards` `Kubernetes`
...
- **Unity → WebGL** build embedded in the mini-app container
...
The interesting problem wasn't the card game. It was making a Unity build, a messenger's auth, and a blockchain agree with each other — reliably, at scale, on two platforms at once.
